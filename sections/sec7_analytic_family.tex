...
The cyclic subspace of a unital star-algebra is reducing.  Positivity and
connectedness give a simple ground state; its projector is a polynomial in
$H_a$ and maps the Dicke state to a nonzero multiple of $\ket{\psi_a}$.
Rayleigh--Ritz on
$\mathcal K_J^{\rm par}\subseteq\mathcal H^{D_n}$ then gives
Eq.~\eqref{eq:parent-cyclic-fixed-comparison}.  Appendix~\ref{app:parent-accessibility}
contains the full argument.
...
the full comparison is the single chain
\begin{multline}
 \Delta_{\rm par}^{\rm cyc}(a)\geq\Delta_{D_n}(a)\\
...
\end{multline}
The first step is cyclic accessibility; the second separates cycle geometry
from zero-range kinetics; and the third combines a three-box log-concave
coupling, the Caputo--Sasada recursion, and a conditional two-box comparison.
Appendix~\ref{app:normalized-gap-proof} proves every normalization factor.
...
\paragraph{Proof idea.}
Since a Johnson move changes $U$ by at most two, differentiating
Eqs.~\eqref{eq:johnson-metropolis} and \eqref{eq:cycle-parent} gives
\begin{equation}
...
$n^{41}(\log n)^2$, divided by $T$.  The last dominates.  The complete
derivative and measurement argument is in Appendix~\ref{app:parent-adiabatic}.
