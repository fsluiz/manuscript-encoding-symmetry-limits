...
Let $n_B$ and $n_T$ be the numbers of bases and tasks, respectively, and
fix a positive integer $k$ of address registers.  The incidence matrix is
$A\in\{0,1\}^{n_B\times n_T}$, with $A_{bt}=1$ when base $b$ covers task
$t$.  We first work in the physical address space
\begin{equation}
...
condition.  Let $\ket e=\sum_{b=1}^{n_B}\ket b$ and let $I_B$ be the
...
The original linear interpolation is parameterized by $s\in[0,1]$ as
\begin{equation}
...
here $I$ is the identity on $\mathcal H_{\rm valid}$.  The dynamically
...
$H$ is invariant, $x\in\mathcal O$, and
...
\end{equation}
The sum is independent of the representative.  The orbit-counting
(Burnside) lemma~\cite{Burnside1911} gives
\begin{equation}
...
\end{equation}
where $G_B$ is the faithful base group in
Eq.~\eqref{eq:faithful-base-group}, $[z^k]$ extracts the coefficient of
$z^k$, and $c_\ell(g)$ counts length-$\ell$ cycles in the base action.  Orbit
